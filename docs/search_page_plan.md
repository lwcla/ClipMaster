...
已完成。当前文档记录第一版搜索页设计、最终实现、来源 App 多选筛选调整，以及本次列表/搜索结果共享 UI 从瀑布流改为竖向侧滑 item 的调整；后续搜索相关需求、方案调整或代码实现偏差都必须同步更新本文档。
...
- 搜索结果显示方式与列表页一致，共用竖向滑动 item；item 保留卡片和边框样式，展示链接、链接标题、预览图、剪贴字符串、来源 App、时间、置顶、删除、复制和进入详情。
- 搜索页根据路由范围切换标题、空态和继续左滑动作：普通搜索继续左滑折叠数据，折叠搜索继续左滑取消折叠。
- 剪贴字符串最多展示三行，超出后省略，避免长文本 item 过高影响列表浏览效率。
- item 最右侧常驻复制按钮，内容区与复制按钮之间使用更细的固定高度分割线，分割线高度比按钮图标大 10dp 并竖向居中；复制按钮在自身点击区域内水平和竖向居中。
- item 向左拖动时，右侧露出置顶/取消置顶和删除按钮；两个按钮高度跟随 item，高度内水平和竖向居中，点击区域保持透明并沿用侧滑操作区整体底色。
- 置顶/取消置顶按钮和删除按钮之间使用同样更细的固定高度分割线，分割线高度比按钮图标大 10dp 并竖向居中；侧滑最大偏移限制为右侧操作区宽度，向右拖动可收回。
- 复制、置顶/取消置顶、删除三个操作按钮宽度统一收敛为 48dp，图标尺寸统一为 24dp，减少操作区对 item 横向空间的占用。
- item 内所有按钮分割线颜色与卡片边框颜色保持一致，避免同一卡片内出现多套边界颜色。
- 置顶状态角标放在卡片内部第一级装饰层，不参与触摸和点击；如果角标与复制按钮区域重叠，复制按钮绘制在角标之上并继续接收点击。
- 不限制同时展开的 item 数量；每个 item 的展开状态按剪贴记录 id 绑定，避免列表复用导致状态串到其他记录。
...
- 现有 `ClipResultList.kt` 已作为列表页和搜索页共享结果组件，本次继续在该文件内集中维护竖向 item 和侧滑操作。
- 共享组件建议包含：
  - `ClipResultList(...)`
  - `ClipCard(...)`
...
- 旧底部三等分操作按钮不再使用；复制改为 item 右侧常驻按钮，置顶和删除改为右侧侧滑操作区按钮。
...
- 列表页和搜索结果页的共享结果 UI 已从双列瀑布流改为 `LazyColumn` 单列 item；首页列表 Tab 仍持有列表滚动状态，重复点击列表 Tab 可回到顶部。
- 共享卡片已改为左侧内容区、更细的固定高度分割线和右侧常驻复制按钮区；复制、置顶/取消置顶、删除三个图标均在各自点击区域内水平和竖向居中。
- 链接预览已支持链接、链接标题和预览图展示；剪贴字符串始终保留但最多显示三行，避免长文本撑高列表项。
- item 左滑时右侧露出置顶/取消置顶和删除按钮，两个按钮点击区保持透明并用固定高度分割线区分；侧滑偏移按剪贴记录 id 保存，不限制多个 item 同时展开。
- 共享 item 已支持第二段继续左滑提示和动作，普通搜索提示“继续滑动折叠数据”，折叠搜索提示“继续滑动取消折叠”。
- 当 Paging refresh 且当前 `itemCount == 0` 时，共享结果列表展示独立 loading，不组合绑定列表状态的空 `LazyColumn`，避免从其他 Tab 切回列表时短暂空刷新窗口把滚动状态钳回顶部。
- 共享卡片内部已改为先测量内容区真实高度，再让复制按钮区和侧滑操作区跟随该高度；避免置顶重排后，小 item 复用旧首条大 item 高度导致显示异常。
- 置顶状态角标已从内容区右上角移到卡片内部第一级装饰层，只作为视觉效果，不参与点击；内容层和复制按钮层绘制在其上方，保证重叠时复制按钮仍优先可见和可点。
- 取消置顶前会记录当前 `LazyListState` 的首个可见 index、offset 和当时的 Paging 快照签名；等异步数据库/Paging 刷新让快照顺序或置顶状态发生变化后，再恢复该 index/offset，避免被移动 item 回到原排序位置时 `LazyColumn` 因稳定 key 锚点自动滚动跟随它。
...
- 竖向侧滑 item 调整后已运行 `./gradlew :app:compileDebugKotlin`，结果通过。
...
- 2026-05-15：将当前状态更新为实现中，并记录列表页和搜索结果页共享 UI 从双列瀑布流改为竖向侧滑 item 的方案；原因是用户希望列表和搜索结果改为单列 item，保留链接预览和剪贴内容，并通过左滑露出置顶、删除操作。
- 2026-05-15：完成竖向侧滑 item 实现并将状态更新为已完成，通过 `./gradlew :app:compileDebugKotlin` 验证，同时按文档约定在顶部补充状态标记；原因是列表页和搜索结果页已共用新的单列 item、常驻复制按钮和右侧侧滑置顶/删除操作。
- 2026-05-15：修复从“我的”切回“列表”时列表回到顶部的问题；原因是竖向 `LazyColumn` 在 Paging refresh 且 `itemCount == 0` 的空刷新窗口绑定同一个列表状态会钳回顶部，最终改为该阶段展示独立 loading，避免空列表接管滚动状态。
- 2026-05-15：将内容区与复制按钮、置顶按钮与删除按钮之间的分割线改为更细的内缩样式；原因是分割线不应占满整个 item 高度，上下各留 10dp 后视觉边界更轻。
- 2026-05-15：取消侧滑置顶和删除按钮的独立背景色；原因是当前交互希望侧滑操作区更克制，只保留图标和内缩分割线表达点击区域。
- 2026-05-15：将三个操作按钮相关分割线从按卡片高度内缩改为固定高度并竖向居中；原因是分割线应围绕按钮视觉中心展示，高度比按钮图标大 10dp 即可表达区域边界。
- 2026-05-15：统一按钮分割线和卡片边框颜色；原因是同一卡片内边界线颜色应保持一致，减少视觉杂色。
- 2026-05-15：缩小复制、置顶/取消置顶和删除三个操作按钮；原因是当前按钮区域视觉占比偏大，统一改为 48dp 点击宽度和 24dp 图标后更轻量。
- 2026-05-15：修复置顶重排后小 item 继承旧首条大 item 高度的问题；原因是原先 `IntrinsicSize.Min` 与按钮 `fillMaxHeight` 的组合在 Lazy item 重排复用时可能保留旧位置测量表现，最终改为内容区先测量真实高度、按钮区跟随内容高度。
- 2026-05-15：调整置顶状态角标层级；原因是角标只是卡片级视觉效果，不应属于内容点击区，也不应覆盖复制按钮点击层。
- 2026-05-15：固定取消置顶后的列表视口位置；原因是 `LazyColumn` 默认会按稳定 key 跟随被移动 item，取消置顶时会把列表带回该 item 的原始位置，最终改为记录操作时的 index/offset 和 Paging 快照签名，等异步排序刷新到达 UI 后再恢复视口。
