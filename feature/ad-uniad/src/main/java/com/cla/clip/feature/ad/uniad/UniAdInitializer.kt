...
/** uni-ad SDK 懒初始化协调器，确保同一会话只启动一次真实初始化。 */
@Singleton
internal class UniAdInitializer @Inject constructor(
    /** uni-ad SDK facade；隔离第三方类型并支持测试替换。 */
    ...
    private fun startInitialization(
        ...
        config: UniAdConfig,
        runtimePolicy: AdRuntimePolicy,
        callbacks: MutableList<UniAdInitCallback>,
    ) {
        runCatching {
            sdkClient.initialize(
                ...
                callback = object : UniAdInitCallback {
                    ...
                    override fun onFailure(reasonCode: String) {
                        /** 失败原因码；空白时回退统一初始化失败。 */
                        val normalizedReason = reasonCode.ifBlank { UniAdReason.INIT_FAILED }
            ...
            state = InitState.Failed(UniAdReason.ADAPTER_EXCEPTION)
            callbacks.toList().forEach { waitingCallback -> waitingCallback.onFailure(UniAdReason.ADAPTER_EXCEPTION) }
            callbacks.clear()
        }
    }
    ...
    private sealed class InitState {
        ...
        data class Starting(
            /** 等待 SDK 初始化完成的回调集合。 */
            val callbacks: MutableList<UniAdInitCallback>,
        ) : InitState()
