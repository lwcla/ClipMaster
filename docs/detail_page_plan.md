...
- 当前剪贴来源包名明确时，提供“屏蔽此应用后续剪贴”或“取消屏蔽此应用”快捷动作；未知来源不展示该动作。
...
- `ClipData`、`ClipCaptureEntity`、Room 表结构、备份恢复协议、列表页和搜索页不在本次改动范围内；`ClipShowEntity` 只新增来源包名字段供详情页动作使用。
...
- 来源包名明确时，底部操作区展示来源过滤说明和快捷动作。点击动作先弹出确认框，文案说明只影响以后保存，不删除当前记录或历史记录。
- 详情页加入或移除过滤名单后只显示轻提示，当前详情页继续停留，不自动删除当前记录。
...
- 详情页订阅 `AppSetting.blockedClipSourcePackagesFlow`，因此从“我的”页或备份恢复改变名单后，返回详情页时动作文案会自动刷新。
- 详情页调用 `AppSetting.addBlockedPackage()` / `removeBlockedPackage()` 原子方法，不新增独立详情设置；成功变化后标记 backup dirty 并调度自动备份。
...
- 手动验证点：来源包名明确的详情页显示屏蔽或取消屏蔽动作，确认后轻提示且页面保留当前记录；未知来源不展示该动作；从我的页修改名单后返回详情页，动作文案刷新。
...
- 来源过滤快捷动作只记录 `detail_block_source_added` / `detail_block_source_removed` 低敏 reasonCode，不记录剪贴正文、完整链接、App 列表或拦截次数。
...
- 本次只为详情页动作在 `ClipShowEntity` 补充来源包名字段；该字段来自已有 `ClipData.sourceAppPackage`，不新增 Room 字段或迁移。
- 不修改 `ClipData`、`ClipCaptureEntity`、DAO、Room schema、备份协议或链接预览缓存；详情页实时从正文提取全部链接，列表页和搜索页继续使用 `clip.link` 展示主链接预览。
...
- 2026-06-17：详情页新增“屏蔽此应用后续剪贴/取消屏蔽此应用”上下文动作接入说明；原因是用户希望从单条历史记录快速维护来源过滤名单，同时历史记录本身不受影响。
