...
已完成。当前文档记录第一版搜索页设计和最终实现，后续搜索相关需求、方案调整或代码实现偏差都必须同步更新本文档。
...
- 来源 App 筛选第一版提供：全部来源和已记录过的具体 App。
...
- 在 `ClipDao` 增加搜索用 `PagingSource<Int, ClipDetail>` 查询，支持以下参数：
  ...
  - 起始时间 `startTime`
  - 结束时间 `endTime`
  - 来源 App 包名 `sourceAppPackage`
- 关键词为空时，按时间和来源 App 条件查询全部匹配记录。
- 关键词非空时，复用现有 FTS 查询思路，并叠加时间和来源 App 条件。
...
- 来源 App 筛选器的具体展示形态可以在实现时根据现有组件风格确定，例如底部弹窗、下拉菜单或筛选 Chip。
