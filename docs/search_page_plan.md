...
- 搜索结果复用共享 `ClipResultList`，卡片结构、左半区动作设置、右滑菜单、复制、置顶、删除、详情跳转和通用侧滑规则以 `docs/clip_result_list_plan.md` 为准。
- 搜索页只维护与搜索相关的差异：传入当前关键词用于可见文本高亮，并根据路由范围切换标题、空态和继续右滑动作；普通搜索继续右滑折叠数据，折叠搜索继续右滑取消折叠。
- 普通搜索结果和普通列表一样读取左半区动作设置；设置不是“无（整卡进入详情）”时启用左右分区，左半区执行复制、置顶、删除或折叠，右半区进入详情。折叠搜索不启用左右分区，不显示左半区背景。
...
- 搜索页通过共享组件参数表达页面差异：`highlightQuery` 传当前关键词；`emptyText` 随普通搜索/折叠搜索切换；`swipePastActionText` 和 `onSwipePastAction` 随 `SearchScope` 切换为折叠或取消折叠；只有普通搜索传入左半区动作设置并启用左右分区。
