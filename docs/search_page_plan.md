...
- item 最右侧常驻复制按钮，内容区与复制按钮之间使用 1dp 分割线；复制按钮在自身点击区域内水平和竖向居中。
- item 向左拖动时，右侧露出置顶/取消置顶和删除按钮；两个按钮高度跟随 item，高度内水平和竖向居中，并通过不同背景色区分点击区域。
- 置顶/取消置顶按钮和删除按钮之间使用 1dp 分割线；侧滑最大偏移限制为右侧操作区宽度，向右拖动可收回。
