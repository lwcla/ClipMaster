...
本轮 UI 刷新后，视频下载页已接入 `SecondaryPageScaffold` 和 `PageBackground`，标题栏与页面背景跟随固定品牌主题；视频提取页因隐藏 WebView、前景遮罩、探测超时和失败重试状态强绑定页面 chrome，本轮暂不迁入通用二级骨架，后续应和图片提取页一起抽象 WebView 探测页骨架。
...
- 任务以 `video_url` 建普通索引，同一视频地址允许产生多条记录，符合下载记录按次数保留的语义；后续如果需要聚合展示，可在记录页增加来源分组字段或分组查询。
...
- 下载记录页已区分“进入下载页查看状态”和“点击重新下载”：查看已成功记录不会自动重置并入队 Worker，只有重新下载才创建新任务并启动下载。
- 视频提取页暂未切入统一二级页面骨架；风险是提取页和其他二级页在背景/内容避让上仍可能存在细节差异，后续应先抽象探测页骨架再迁移，避免破坏 WebView 生命周期和遮罩交互。

## 变更记录

- 2026-05-27：视频下载页接入统一二级页面骨架，并记录视频提取页保留专属探测 chrome 的例外；原因是下载页只是状态展示和 Worker 观察，低风险适合统一骨架，提取页则绑定隐藏 WebView 与探测状态机，需要下一轮单独抽象。
