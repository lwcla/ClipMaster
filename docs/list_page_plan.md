...
列表页是首页底部 Tab 的默认一级页面，负责展示本地未折叠且未进入回收站的剪贴记录、进入搜索页、进入详情页，以及承接复制、删除、置顶/取消置顶和折叠等剪贴记录操作。当前列表页使用 `TopLevelPageScaffold`、固定一级标题、标题栏右侧搜索入口、Shizuku 状态提示，并通过共享 `ClipResultList` 渲染剪贴结果。
