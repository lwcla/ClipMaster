...
- 新增或修改的类型、字段、方法、构造函数、扩展函数、mapper、parser、formatter、validator、helper 和工具函数都必须补充贴近声明的简体中文注释。
- 新增或修改的每个变量声明都必须补充简体中文注释，包括局部变量、字段、常量、配置项、阈值、状态字段、缓存字段、锁、集合、正则、脚本、SQL 片段和协议 key。
- 方法注释不区分可见性，`public`、`internal`、`protected`、`private` 和文件内辅助函数都按同一标准说明输入输出、调用时机、失败契约、线程/协程边界和为什么放在当前层级。
- 变量注释要说明业务用途、单位、取值依据、有效范围、读写方、持久化/序列化约束或修改风险；同一语义组的多个局部变量可以共用紧邻注释，但必须逐一说明每个变量含义。
- 关键分支、提前返回、兜底逻辑、边界条件、重试、超时、去重、排序、过滤、限流、并发控制、线程切换、资源释放、权限判断、版本兼容、数据迁移、数据库事务、幂等性和非直观实现必须补充简体中文注释说明原因。
- 避免语法复述式注释，但不能把“命名自解释”“逻辑很短”“只是局部变量”作为省略注释的理由；注释至少要补出业务含义、边界或修改风险。
