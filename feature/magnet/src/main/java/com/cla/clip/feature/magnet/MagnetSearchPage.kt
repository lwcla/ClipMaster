...
import com.cla.clip.feature.magnet.source.cache.MagnetSourceSearchRepository
import com.cla.clip.feature.magnet.source.cache.MagnetSourceStatusReason
import com.cla.clip.feature.magnet.source.cache.MagnetSourceSyncPhase
import com.cla.clip.feature.magnet.source.cache.MagnetSourceSyncProgress
...
fun MagnetSearchPage(
    ...
    Scaffold(
        topBar = {
            MagnetFeatureTitleBar(
                title = stringResource(R.string.magnet_feature_magnet_search),
                onBack = onBack
            )
        },
        ...
        Column(
            ...
            MagnetSearchInputField(
                ...
                placeholder = stringResource(R.string.magnet_feature_search_magnet_hint),
                clearContentDescription = stringResource(BaseR.string.base_general_clear_search_keyword),
            )
        ...
        Column {
            Row(verticalAlignment = Alignment.CenterVertically) {
                Column(modifier = Modifier.weight(1f)) {
                    ...
                    Text(
                        text = stringResource(R.string.magnet_feature_magnet_privacy_notice),
                ...
                Button(
                    ...
                    Text(stringResource(R.string.magnet_feature_magnet_sync_source))
                }
            }
            ...
            if (syncing) {
                ...
                TextButton(onClick = onForceSync) {
                    Text(stringResource(R.string.magnet_feature_magnet_force_sync_source))
                }
            }
        }
    }
...
private fun MagnetResultList(
    ...
    val retryText = stringResource(BaseR.string.base_general_data_load_failed_retry)
    when {
        !cacheReady -> MagnetPagingEmptyContent(
            text = stringResource(R.string.magnet_feature_magnet_source_need_sync),
            icon = Icons.Default.CloudSync
        )
        query.length < MagnetSourceSearchRepository.MIN_SEARCH_QUERY_LENGTH -> MagnetPagingEmptyContent(
            text = stringResource(R.string.magnet_feature_magnet_query_too_short),
            icon = Icons.Default.Search
        )
        pagedResults.loadState.refresh is LoadState.Loading && pagedResults.itemCount == 0 -> MagnetPagingLoadingContent()
        pagedResults.loadState.refresh is LoadState.Error && pagedResults.itemCount == 0 -> MagnetPagingErrorContent(
            text = retryText,
            onRetry = pagedResults::retry
        )
        pagedResults.loadState.refresh is LoadState.NotLoading && pagedResults.itemCount == 0 -> MagnetPagingEmptyContent(
            text = stringResource(R.string.magnet_feature_magnet_result_empty),
            icon = Icons.Default.Search
        )
        else -> {
            LazyColumn(
...
private fun MagnetSourceSyncProgress.progressText(): String {
    return when (phase) {
        MagnetSourceSyncPhase.Checking -> stringResource(R.string.magnet_feature_magnet_sync_checking)
        MagnetSourceSyncPhase.Downloading -> stringResource(R.string.magnet_feature_magnet_sync_downloading)
        MagnetSourceSyncPhase.Parsing -> stringResource(R.string.magnet_feature_magnet_sync_parsing, parsedCount)
        MagnetSourceSyncPhase.Importing -> stringResource(R.string.magnet_feature_magnet_sync_importing, importedCount)
        MagnetSourceSyncPhase.Completed -> stringResource(R.string.magnet_feature_magnet_sync_completed)
        MagnetSourceSyncPhase.Failed -> stringResource(R.string.magnet_feature_magnet_sync_failed)
        MagnetSourceSyncPhase.Cancelled -> stringResource(R.string.magnet_feature_magnet_sync_cancelled)
        MagnetSourceSyncPhase.Idle -> stringResource(R.string.magnet_feature_magnet_sync_idle)
    }
}
