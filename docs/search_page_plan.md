...
- item 最右侧常驻复制按钮，内容区与复制按钮之间使用更细的内缩分割线，分割线上下各留 10dp；复制按钮在自身点击区域内水平和竖向居中。
- item 向左拖动时，右侧露出置顶/取消置顶和删除按钮；两个按钮高度跟随 item，高度内水平和竖向居中，点击区域保持透明并沿用侧滑操作区整体底色。
- 置顶/取消置顶按钮和删除按钮之间使用同样更细的内缩分割线，分割线上下各留 10dp；侧滑最大偏移限制为右侧操作区宽度，向右拖动可收回。
...
- 共享卡片已改为左侧内容区、1dp 分割线和右侧常驻复制按钮区；复制、置顶/取消置顶、删除三个图标均在各自点击区域内水平和竖向居中。
- 链接预览已支持链接、链接标题和预览图展示；剪贴字符串始终保留但最多显示三行，避免长文本撑高列表项。
- item 左滑时右侧露出置顶/取消置顶和删除按钮，两个按钮使用不同背景色并用 1dp 分割线区分；侧滑偏移按剪贴记录 id 保存，不限制多个 item 同时展开。
