...
2026-05-18 开始调整图片提取探测阶段的前台体验：隐藏 `WebView` 仍负责真实网页加载、自动滚动、DOM 扫描和网络拦截，但用户前台不再只看到加载态。只要探测到第一张有效图片，页面就切换到实时候选网格，实时展示已获取图片、默认选中新增图片，并允许用户取消选择、预览候选、手动结束提取后再下载。自动完成和用户手动结束都只表示探测停止，页面停留在选择确认状态，不自动创建批次或进入下载；只有用户再次确认下载时才把已选候选写入 Room 并启动 Worker。
...
本轮 UI 刷新保留图片提取页现有探测 chrome，因为透明 WebView、实时网格、手动结束、重新提取确认和下载提交状态共同决定页面结构；页面继续复用 `TitleBar`、共享媒体状态/预览组件和固定品牌主题，后续应和视频提取页一起抽象 WebView 探测页骨架后再迁入统一二级页面骨架。
...
2. 发现第一张有效图片后，页面立即展示实时候选网格；顶部第一行左侧显示“正在提取中/提取已完成”和已选数量，右侧固定放置主按钮“结束提取/下载已选图片”，第二行提供重新提取、全选和取消全选，避免所有按钮挤在同一行。
...
8. 用户点击“结束提取”后，页面只进入停止提取状态，取消滚动协程、停止后台 WebView，并冻结当前候选和选择状态；此时不创建批次、不启动下载。
9. 如果自动完成探测或用户手动结束提取，页面顶部切换为“提取已完成”，关闭后台 WebView，仍停留在网格中等待用户点击“下载已选图片”，不会自动下载。
10. 用户在“提取已完成”状态点击“下载已选图片”后，才把当前已选候选写入批次并启动 Worker；提交失败时回到当前网格并保留选择状态。
11. 正在提取或自动完成后已有候选但未下载时，点击重新提取需要二次确认；确认后清空当前候选和选择状态并启动全新会话。停止提取和提交下载中禁用重新提取；下载开始后的重新提取不影响已创建的 Worker 批次。
12. 下载完成后，用户点击完成态文案会直接打开系统相册；如果相册支持 bucket 参数，会尽量定位到 `clipMaster/<本次网页标题目录>` 对应相册。
13. 点击图片下载结果通知时，同样直接打开相册；如果没有任何图片成功保存，则不携带具体目录，避免误导用户进入空目录。
...
- 图片预览弹窗、可选图片缩略图、Coil 动图 ImageLoader、图片加载请求头、预览宽高比和元信息格式化已收敛到共享媒体 UI 组件；`ImagePreviewComponents.kt` 只保留图片提取页实体到共享组件契约的适配，完整共享规则见 `docs/shared_media_ui_plan.md`。
...
- 探测阶段维护内存中的阶段性候选列表，供长耗时提示、进度查看和手动结束使用；阶段性候选不写入 Room，自动完成或用户点击“结束提取”时也只冻结内存候选，只有用户在待下载状态点击“下载已选图片”才创建正式批次。
...
- 进度查看页只读取阶段性候选，不写数据库、不启动下载；用户点击“结束提取”后只取消探测、销毁 WebView、冻结当前候选并进入最终选择页。
...
- 点击“查看已获取图片”后能看到当前阶段性缩略图，普通返回后探测继续；点击“结束提取”后用当前候选进入最终选择页但不启动下载。
...
- 用户在探测中取消图片并滚动到列表中部后，自动完成只应把顶部状态切换为“提取已完成”，不应恢复全选，也不应把网格滚回顶部。
...
- 点击“结束提取”后只停止识别并保留当前候选、选择状态和滚动位置，不创建批次、不启动下载；随后点击“下载已选图片”才只创建一次批次并下载已选图片，提交失败后回到选择网格，候选和选择状态不丢失。
...
- 图片提取页暂未切入统一二级页面骨架；风险是探测页与其他二级页的背景/内容避让仍可能有细节差异，后续应先抽象可复用的 WebView 探测页骨架，避免影响透明 WebView、实时网格和重新提取确认流程。
...
- 后续评估预览底部弹窗高度是否需要随图片内容自适应。当前弹窗外层固定最高占屏幕 86%，图片区使用剩余空间，因此不同尺寸图片的弹窗整体高度看起来接近一致；若后续用户更关注小图轻量预览，可考虑按图片宽高比、元信息区高度和屏幕上限计算自适应高度，但需要继续保证长图可滚动查看、动图播放稳定和底部操作区不被挤出。
...
- 2026-05-27：记录图片提取页在 UI 刷新中的骨架例外；原因是透明 WebView、实时选择网格和提交流程绑定页面 chrome，本轮只复用主题、标题栏和共享媒体组件，后续与视频提取页一起抽象探测页骨架后再迁入。
- 2026-05-18：修正实时提取中新识别图片未立即默认选中的问题；原因是实时候选列表对象会复用，`LaunchedEffect(candidates)` 不会因列表内容追加而重新执行，最终改为使用候选稳定 key 快照触发增量默认选中，同时继续保留用户手动取消过的候选。
- 2026-05-18：修正图片探测完成后实时选择状态和滚动位置被重置的问题；原因是探测中和待下载状态此前在不同分支分别创建 `LiveCandidateSelectionContent`，状态切换会销毁本地选择集合和网格滚动状态，最终改为共用同一个调用位置并只切换提取状态参数。
...
- 2026-05-18：将图片预览、可选缩略图、动图 ImageLoader 和行内状态提示进一步接入共享媒体 UI 组件；原因是图片提取页和下载记录页存在相同的图片预览/缩略图能力，不能只按页面目录局部拆分，最终以 `docs/shared_media_ui_plan.md` 的共享组件契约为准。
