...
- 列表内容复用 `ClipResultList` 单列竖向 item，具体卡片展示、左半区动作设置、右滑菜单和第二段滑动动画见 `docs/clip_result_list_plan.md`。
...
- 普通列表读取“我的”页保存的左半区动作设置；当设置不是“无（整卡进入详情）”时启用左右等分点击区并显示左半区同源浅色背景，左半区执行复制、置顶/取消置顶、删除或折叠，右半区进入详情。
- 设置为“无（整卡进入详情）”时，普通列表不显示左半区背景、不启用左右分区，整卡点击进入详情；右滑菜单仍保留复制、置顶/取消置顶和删除。
