...
当前仓库没有历史广告实现。本轮新增 `:feature:ad-api` 作为稳定广告抽象，新增 `:feature:ad-debug` 作为调试占位实现，并在详情页接入一个信息流/原生广告位。国内广告第一家 adapter 已确定为穿山甲 CSJ/Pangle，新增 `:feature:ad-csj`；当 `local.properties` 或 CI Gradle 属性按 buildType 配置 debug 或 release 对应 AppId 与详情页广告位 ID 时，对应构建类型默认把该模块编译进 app。临时关闭某个构建类型的广告模块时，注释掉对应 buildType 的 AppId 或广告位 ID 即可。广告 SDK 不在 `Application.onCreate()` 初始化，不向普通用户暴露广告源选择 UI。
...
- 默认构建在未配置广告参数时仍不接真实广告 SDK；debug/internal 通过本机/CI 配置 `csjDebugAppId` 和 `csjDebugDetailNativeAdSlotId` 自动编译进穿山甲 adapter，release 通过 `csjReleaseAppId` 和 `csjReleaseDetailNativeAdSlotId` 自动编译进穿山甲 adapter。临时关闭广告模块时注释掉对应 AppId 或广告位 ID。
