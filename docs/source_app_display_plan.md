...
- 如果已选来源包名不在当前候选列表里，搜索页来源选择器继续回退显示包名，避免隐藏已有筛选条件。
...
- 2026-05-31：同步搜索页来源入口命名为来源选择器；原因是搜索页筛选区已从 Chip 形态改为等宽选择器，文档需要避免保留过期 UI 形态描述。
