...
- 恢复前会先创建安全快照：优先保存到用户已授权的本地备份文件夹；未设置本地目录时保存到应用私有目录。安全快照失败时阻止恢复，避免没有回滚点就写库。
...
6. 自动备份成功后按 manifest 中的 `backupKind`、安装标识和创建时间清理旧远端自动备份；手动 WebDAV 备份不触发普通保留清理。
...
1. 普通自动备份默认保留 5 份，用户可设置 1-20。
2. 清理只作用于 `backupKind = auto` 且属于当前设备/当前安装标识的备份，不自动删除手动备份、安全快照、其他设备备份或用户目录里的其他文件。
3. 新备份完整写入、校验和发布成功后才清理旧备份，避免新备份失败时旧备份也被删除。
4. 排序优先使用 manifest `created_at`，缺失时使用文件名时间戳或服务端修改时间兜底。
5. `latest.json` 不计入保留份数。
6. 每次自动备份成功后记录本地/WebDAV 清理了多少旧备份，最近成功摘要中展示清理结果。

### 恢复前安全快照

1. 用户确认恢复后、正式写库事务开始前，先用当前数据创建 `backupKind = safety` 的安全快照。
2. 安全快照文件名带 `safety` 标识，manifest 也写入 `backupKind = safety`，列表和报告都明确这是恢复前回滚点。
...
- 自动备份、手动本地备份、手动 WebDAV 备份、恢复、安全快照、保留清理和 WebDAV 健康检查都生成短 `taskId`，同一次流程的开始、关键阶段、成功、失败、跳过和重试日志必须带同一个 `taskId`。
...
- 恢复流程记录 `restore start/success/failed`，字段包括备份类型、文件大小、预检数量摘要、新增/更新/跳过数量、安全快照文件名和耗时；失败日志输出 reasonCode 和异常类型，不输出备份内容。
- 恢复前安全快照记录目标类型、本地/私有目录写入、固定保留 3 份清理数量、文件名、文件大小和耗时；不记录应用私有目录绝对路径或 SAF URI 原文。
...
- `logD` 用于阶段性流程、调度和数量摘要；`logI` 用于用户触发或后台关键任务成功；`logW` 用于跳过、部分成功、可恢复重试或 WebDAV 健康不可用；`logE` 用于任务失败、安全快照失败、恢复被阻止或清理删除失败。
...
- 新增 `backupKind = manual / auto / safety`，用于列表展示、保留清理和安全快照识别。
- 新增自动备份统一开关、保留份数 1-20、仅 Wi-Fi、dirty 状态、最近自动备份状态和最近成功摘要。
- 新增 WorkManager 自动备份任务：每日兜底、dirty 延迟触发、任务互斥、失败退避、跳过/失败状态区分。
- 新增本地和 WebDAV 自动备份保留清理；只清理当前安装标识的自动备份，且只在新备份成功后清理旧备份。
- 新增恢复前安全快照；优先写本地授权目录，兜底写应用私有目录，失败时阻止恢复。
- 新增本地备份目录列表，展示自动备份和安全快照并支持预览。
...
- 恢复前安全快照不上传 WebDAV，降低恢复等待时间和明文云端扩散风险。
