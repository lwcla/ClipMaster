...
- 复制作为普通主操作展示；删除作为危险操作单独放在底部文本按钮中。
...
- 手动验证点：详情页加载态、错误态、正文滚动、复制、危险删除入口、删除选择弹窗、图片提取和视频提取入口行为不变。
