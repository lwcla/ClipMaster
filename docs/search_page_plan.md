...
已完成。当前文档记录第一版搜索页设计、最终实现、来源 App 多选筛选调整，以及搜索结果复用 `ClipResultList` 的页面差异。`ClipResultList` 的卡片结构、按钮布局、侧滑方向、动画阈值和通用测试验证以 `docs/clip_result_list_plan.md` 为主文档；后续搜索相关需求、筛选、高亮、搜索范围或搜索页差异行为发生变化时，必须同步更新本文档。
...
- 删除、复制、置顶和点击详情继续通过回调传入，避免共享组件直接依赖搜索页 ViewModel。
